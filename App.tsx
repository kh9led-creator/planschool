...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // Only initialize if config is valid (check for apiKey at least)
    if (config.apiKey && config.apiKey !== 'undefined') {
        const connected = initFirebase(config);
        setIsCloud(connected);
    }
  }, []);
  ...
  useEffect(() => {
    const loadRegistry = async () => {
        ...
        if (isCloud && getDB()) {
            const cloudData = await loadSystemData('schools_registry');
            if (Array.isArray(cloudData)) {
                loadedSchools = cloudData;
                // Sync to local as backup
                localStorage.setItem('system_schools', JSON.stringify(cloudData));
            }
        } else {
            // Fallback to local
            const localData = localStorage.getItem('system_schools');
            if (localData) {
                try {
                    loadedSchools = JSON.parse(localData);
                } catch (e) {
                    console.error("Error parsing local schools", e);
                }
            }
        }
        ...
        // Restore last active school
        const lastActiveId = localStorage.getItem('last_active_school_id');
        if (lastActiveId && loadedSchools.some(s => s.id === lastActiveId)) {
            setSelectedSchoolId(lastActiveId);
  ...
  const handleRegisterSchool = async (data: Partial<SchoolMetadata>) => {
      const newSchool: SchoolMetadata = {
          ...
          isActive: true,
          subscriptionEnd: new Date(Date.now() + 14 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], // 14 days trial
          plan: 'trial',
          licenseKey: Math.random().toString(36).substring(2, 12).toUpperCase(),
  ...
  const handleUpgradeSubscription = async (id: string, plan: SubscriptionPlan, code: string) => {
      ...
      const updatedSchool = {
          ...school,
          plan: plan,
          subscriptionEnd: baseDate.toISOString().split('T')[0],
          isActive: true,
          isPaid: true
      };
  ...
  const pricing: PricingConfig = { quarterly: 150, annual: 450, currency: 'SAR' };
          ...
          <ErrorBoundary>
              <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-4">
                  <div className="bg-white p-8 rounded-2xl shadow-xl border border-slate-100 max-w-md w-full text-center">
                      ...
                      <h2 className="text-2xl font-bold text-slate-800 mb-2">النظام جديد</h2>
                      <p className="text-slate-500 mb-8">لم يتم العثور على أي مدارس مسجلة. قم بتسجيل مدرستك الأولى للبدء.</p>
                      <button 
                         onClick={() => {
                             const name = prompt('الرجاء إدخال اسم المدرسة:');
                             if(name && name.length > 2) handleRegisterSchool({ name });
                         }}
  ...
  return (
      <ErrorBoundary>
          <SchoolSystem 
              ...
              onOpenSystemAdmin={() => alert("قريباً: لوحة تحكم النظام المركزي")}
